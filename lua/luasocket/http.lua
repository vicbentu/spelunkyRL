...
local headers = require("luasocket.headers")
local base = _G
...
function metat.__index:receivestatusline()
    ...
    if status ~= "HTTP/" then
        if ec == "timeout" then return 408 end
        return nil, status
    end
...
local function adjustheaders(reqt)
    ...
    if port ~= tostring(SCHEMES[reqt.scheme].port) then host = host .. ':' .. port end
    ...
    for i, v in base.pairs(reqt.headers or lower) do lower[string.lower(i)] = v end
    return lower
end
...
local function adjustrequest(reqt)
    ...
    if not (host and host ~= "") then socket.try(nil, "invalid host '" .. base.tostring(nreqt.host) .. "'") end
...
local function shouldredirect(reqt, code, headers)
    ...
    return (reqt.redirect ~= false) and (code == 301 or code == 302 or code == 303 or code == 307) and
               (not reqt.method or reqt.method == "GET" or reqt.method == "HEAD") and
               ((false == reqt.maxredirects) or ((reqt.nredirects or 0) < (reqt.maxredirects or 5)))
end
...
function trequest(reqt)
    ...
    if shouldreceivebody(nreqt, code) then h:receivebody(headers, nreqt.sink, nreqt.step) end
    h:close()
