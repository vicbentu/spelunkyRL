...
local function make_set(t)
    local s = {}
    for i, v in base.ipairs(t) do s[t[i]] = 1 end
    return s
end
...
local function absolute_path(base_path, relative_path)
    if string.sub(relative_path, 1, 1) == "/" then return remove_dot_components(relative_path) end
...
function _M.build(parsed)
    ...
    if parsed.host then
        ...
        if parsed.user then
            ...
            if parsed.password then userinfo = userinfo .. ":" .. parsed.password end
        end
        ...
        if not relative_parsed.authority then
            ...
            if not relative_parsed.path then
                ...
                if not relative_parsed.params then
                    ...
                    if not relative_parsed.query then relative_parsed.query = base_parsed.query end
                end
            else
...
function _M.parse_path(path)
    ...
    for i = 1, #parsed do parsed[i] = _M.unescape(parsed[i]) end
